...
// PUT - add favouriteFriend --------------------------------------
router.put('/addFavFriend/', Utils.authenticateToken, (req, res) => {  
  // validate check
  ...
  // add friendId to favouriteHaircuts field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      favouriteFriends: req.body.friendId
    }
  })
    .then((user) => {            
      res.json({
    ...
    .catch(err => {
      console.log(err)
      res.status(500).json({
        message: "Problem adding favourite friend"
      })
    })
})
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouriteFriends')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
